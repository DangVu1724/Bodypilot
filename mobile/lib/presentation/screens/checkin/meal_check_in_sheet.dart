...
class _MealCheckInSheetState extends State<MealCheckInSheet> {
  ...
  final List<Map<String, String>> _newGoalOptions = [
    {'value': 'MAINTAIN', 'label': 'Duy trì vóc dáng hiện tại'},
  ...
  void initState() {
    super.initState();
    _selectedWeight = widget.currentWeight > 0 ? widget.currentWeight : 65.0;
    _weightController = TextEditingController(text: _selectedWeight.toStringAsFixed(1));
  }
  ...
  void _submitMealCheckIn() async {
    final weight = double.tryParse(_weightController.text) ?? _selectedWeight;
    if (weight <= 0) {
    ...
    try {
      ...
      if (result != null) {
        await TokenService.setMealCheckInDone(true, summary: {
          'weight': result.newWeight,
          'weightChange': result.weightChange,
          'avgCalories': (result.newTargetCalories * 0.95).toInt(),
          'targetCalories': result.newTargetCalories.toInt(),
          'avgProtein': (result.newTargetCalories * 0.25 / 4).toInt(),
          'targetProtein': (result.newTargetCalories * 0.28 / 4).toInt(),
  ...
  Widget _buildWeightStepperInput() {
    return Container(
            ...
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: const Color(0xFFFF7A30).withValues(alpha: 0.12), borderRadius: BorderRadius.circular(14)),
                ...
                Expanded(
                  child: InkWell(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: _isGoalCompleted == false ? AppTheme.primary.withValues(alpha: 0.12) : const Color(0xFFF8FAFC),
                        borderRadius: BorderRadius.circular(14),
                ...
                Expanded(
                  child: InkWell(
                    ...
                    child: Container(
                      padding: const EdgeInsets.symmetric(vertical: 14),
                      decoration: BoxDecoration(
                        color: _isGoalCompleted == true ? Colors.green.withValues(alpha: 0.12) : const Color(0xFFF8FAFC),
                        borderRadius: BorderRadius.circular(14),
                ...
                children: _newGoalOptions.map((opt) {
                  ...
                  return Container(
                    ...
                    child: ListTile(
                      ...
                      tileColor: selected ? AppTheme.primary.withValues(alpha: 0.08) : const Color(0xFFF8FAFC),
