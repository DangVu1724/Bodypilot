...
public class LlmRouterService {
    ...
    public String routeChatRequest(String selectedModel, String prompt, String systemInstruction, boolean forceJson) throws Exception {
        String model = (selectedModel != null && !selectedModel.trim().isEmpty()) ? selectedModel.trim().toLowerCase() : "gemini-2.5-flash";

        log.info("🤖 [LLM_ROUTER] Routing request for model (Routed to Gemini 2.5 Flash): '{}', forceJson: {}", model, forceJson);
        ...
        if (model.contains("groq") || model.contains("llama")) {
            return callGroqWithFallback(selectedModel != null ? selectedModel.trim() : "llama-3.1-8b-instant", prompt, systemInstruction, forceJson);
        } else {
            return callGeminiWithFallback(prompt, systemInstruction, forceJson);
        }
        */
    ...
    private String callGroqWithFallback(String modelName, String prompt, String systemInstruction, boolean forceJson) throws Exception {
        ...
        if (groqClient.isApiKeyConfigured()) {
            try {
                ...
                log.warn("⚠️ [LLM_ROUTER] Groq model '{}' failed: {}. Attempting fallback to Gemini...", modelName, e.getMessage());
            ...
            log.warn("⚠️ [LLM_ROUTER] Groq API key not configured. Falling back to Gemini...");
            return geminiClient.callGemini(prompt, systemInstruction, forceJson);
        } else {
            throw new IllegalStateException("Cấu hình API Key AI (Groq & Gemini) chưa sẵn sàng.");
        }
        */
    }
