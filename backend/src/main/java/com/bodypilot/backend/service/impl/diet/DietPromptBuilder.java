...
/**
 * Component dedicated to building AI Diet Prompt strings with Vietnamese cultural food context.
 */
@Component
...
public class DietPromptBuilder {
    ...
    public String buildPrompt(UserProfile profile, UserGoal goal, UserMetricHistory metric,
        ...
        sb.append("Tạo kế hoạch dinh dưỡng gợi ý trong ").append(days).append(" ngày liên tiếp bắt đầu từ ngày ")
                .append(startDate)
        ...
        if (allergies != null && !allergies.isEmpty()) {
            String allergyList = allergies.stream()
                    .map(a -> a.getAllergyMaster() != null ? a.getAllergyMaster().getName() : a.getNote())
                    .filter(s -> s != null && !s.isEmpty())
                    .collect(Collectors.joining(", "));
            sb.append("- Dị ứng thực phẩm (BẮT BUỘC TRÁNH KHỎI THỰC ĐƠN): ").append(allergyList).append("\n");
        }
        ...
        if (dislikes != null && !dislikes.isEmpty()) {
            String dislikeList = dislikes.stream()
                    .map(d -> d.getDislikedFoodGroup() != null ? d.getDislikedFoodGroup().name() : d.getNote())
                    .filter(s -> s != null && !s.isEmpty())
                    .collect(Collectors.joining(", "));
        ...
        if (userFeedback != null && !userFeedback.trim().isEmpty()) {
            sb.append("\n⚠️ GHI CHÚ / YÊU CẦU ĐẶC BIỆT TỪ NGƯỜI DÙNG CHO LẦN TẠO NÀY:\n");
